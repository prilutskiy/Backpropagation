...
namespace Backpropagation.Core
{
    ...
    public class NeuralNetwork
    {
        ...
        public NeuralNetwork(int inputCount, int outputCount, Func<Double[], Double[], Double> activationFunction,
            int layerCount = 2)
        {
        ...
        public Double[] GetNetworkOutput(INeuralImage image)
        {
            ...
            //all hidden layers
            for (var i = 1; i < NeuralLayers.Count() - 1; i ++)
            {
        ...
        /// <returns></returns>
        public int TrainNetwork(ICollection<INeuralImage> imgs, int iterationsLimit = -1,
            Action<int> trainCallBack = null)
        {
            InitNeuralLayers();
            var iterations = 0;
            bool noErrors;
            do
            {
                ...
                foreach (var img in imgs)
                {
                    ...
                    for (var i = 0; i < recResult.Count(); i++)
                        outputDeviation[i] = outputRule[i] - roundedOutput[i];
                    SetNeuronErrorSignals(outputDeviation);
                        //вычислить коэф. ошибки для каждого нейрона каждого слоя (начиная с выходного слоя
        ...
        private void SetNeuronErrorSignals(double[] outputDeviation)
        {
            ...
            foreach (var neuron in outputLayer.Neurons)
                //устанавливаем коэф-ты ошибки нейронов в выходном слое, основываясь на полученных выходных значениях
                neuron.ErrorSignal = outputDeviation[x++];
            for (var nc = NeuralLayers.Count - 1; nc > 0; nc--) //проходим по всем слоям, начиная с выходного
            {
                var layer = NeuralLayers.ToList()[nc]; //текущий слой
                var subLayer = NeuralLayers.ToList()[nc - 1]; // предыдущий слой
                for (var i = 0; i < layer.Neurons.Count; i++) //проходим по всем нейронам в выбранном слое
                {
                    for (var j = 0; j < subLayer.Neurons.Count; j++) //проходим по всем нейронам в предыдущем слое
                    {
                        var neuron = layer.Neurons.ToList()[i]; //нейрон в основном слое
                        var subNeuron = subLayer.Neurons.ToList()[j]; // нейрон в подслое
                        subNeuron.ErrorSignal += neuron.ErrorSignal*neuron.WeightFactors[j];
                            //устанавливаем коэф. ошибки нейрона в подслое, основываясь на коэф.-те ошибки нейрона в основном слое
                    }
                }
            }
        }
